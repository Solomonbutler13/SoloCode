...
const numbersGreaterThan100 = dataset.filter(number => number > 100);

console.log('Numbers larger than 100:', numbersGreaterThan100);
```
...
const answerFormat = names.map(name => ({
  name: name,
  age: Math.floor(Math.random() * 100) + 1 // Generates a random number between 1 and 100
}));
...
const cars = [
  ...
  const carsWithRecall = cars.filter(car => car.activeRecall);

console.log('Cars with Active Recall:', carsWithRecall);
```
...
const areaCodes = {
  ...
  const organizedPhoneNums = phoneNums.reduce((result, phoneNumber) => {
    ...
    if (areaCodes.hasOwnProperty(areaCode)) {
      ...
      result[areaCode].push(phoneNumber);
    }
  
    return result;
  }, {});
  
  console.log('Organized Phone Numbers:', organizedPhoneNums);
```
...
const roster = [
  ...
  const injuredPlayers = roster.reduce((result, player) => {
    if (player.injured) {
      result.push(player);
    }
    return result;
  ...
  console.log('Injured Players (Alphabetical Order):', injuredPlayers);
```
...
const playersByPosition = roster.reduce((result, player) => {
  if (!player.injured) {
    if (!result[player.position]) {
      result[player.position] = [];
    }
    result[player.position].push(player.name);
  }
  return result;
}, {});

console.log('Players by Position (Not Injured):', playersByPosition);
```
...
const sumOfNumbers = nums2.reduce((sum, num) => sum + num, 0);
...
const marines = [
  ...
  const isActiveQualification = (marine) => marine.weaponsQualification.expirationDate > new Date();


const sortByScoreDescending = (a, b) => b.weaponsQualification.score - a.weaponsQualification.score;


const activeMarines = marines.filter(isActiveQualification).sort(sortByScoreDescending);

console.log('Active Marines with Weapons Qualification (Sorted by Score):', activeMarines);
```