...
console.log(
    people.filter(person => person.birthYear === 2000)
)
...
console.log(
    phoneNums.map(phoneNum => [ phoneNum[0], '##-###-#', ...phoneNum.slice(9) ].join(''))
)
...
console.log(
    peopleArray.filter(person => person.length > 5)
)
...
// Filter out people under 20 or over 35
people1 = people1.filter(person => person.age > 20 && person.age < 35)
